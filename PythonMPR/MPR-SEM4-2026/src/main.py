...
def configure_styles():
    ...
    style.map("Constraint.TCombobox",
             foreground=[("focus", "#1e293b"), ("!focus", "#1e293b")],
             fieldbackground=[("focus", "white"), ("!focus", "white")],
             background=[("focus", PRIMARY_COLOR), ("!focus", "white")])
...
def add_constraint():
    ...
    plus_label = tk.Label(constraints_frame_container, text="+", font=LABEL_FONT,
                         bg=SURFACE_COLOR, fg=TEXT_COLOR)
    b = tk.Entry(constraints_frame_container, width=8, font=DEFAULT_FONT,
                relief="groove", bd=2, bg="white", highlightthickness=1, highlightcolor=PRIMARY_COLOR)
    y_label = tk.Label(constraints_frame_container, text="y", font=LABEL_FONT,
                      bg=SURFACE_COLOR, fg=TEXT_COLOR)

    sign_var = tk.StringVar(value="≤")
    sign_menu = ttk.Combobox(constraints_frame_container, textvariable=sign_var, 
                            values=["≤", "≥", "<", ">"], state="readonly",
                            style="Constraint.TCombobox", width=8)
    sign_menu.current(0)  # Set default to first value

    c = tk.Entry(constraints_frame_container, width=8, font=DEFAULT_FONT,
                relief="groove", bd=2, bg="white", highlightthickness=1, highlightcolor=PRIMARY_COLOR)

    delete_btn = tk.Button(constraints_frame_container, text="🗑️", bg="#f87171", fg="white",
                          font=("Segoe UI", 9), relief="flat", bd=0, padx=4, pady=1,
                          highlightthickness=0, activebackground="#dc2626", activeforeground="white",
                          command=lambda: remove_constraint(a))
    delete_btn.bind("<Enter>", lambda e: delete_btn.config(bg="#ef4444"))
    delete_btn.bind("<Leave>", lambda e: delete_btn.config(bg="#f87171"))
...
def _configure_scroll_region(event=None):
    ...
    main_canvas.itemconfig(main_canvas_window, width=main_canvas.winfo_width())
...
constraints_frame_container.grid_columnconfigure(1, weight=0)
constraints_frame_container.grid_columnconfigure(2, weight=0)
constraints_frame_container.grid_columnconfigure(3, weight=0)
constraints_frame_container.grid_columnconfigure(4, weight=0)
constraints_frame_container.grid_columnconfigure(5, weight=0)
constraints_frame_container.grid_columnconfigure(6, weight=0)
constraints_frame_container.grid_columnconfigure(7, weight=0)
constraints_frame_container.grid_columnconfigure(8, weight=0)
...
add_btn = tk.Button(constraints_frame_container, text="Add Constraint",
                    bg="#87CEEB", fg="white", font=BUTTON_FONT,
...
# Solve button
solve_btn = tk.Button(objective_frame_container, text="Solve Problem",
                      bg=SUCCESS_COLOR, fg="white", font=BUTTON_FONT,
                      relief="flat", bd=0, padx=10, pady=8,
                      command=run_solver)
solve_btn.pack(fill="x", pady=(10, 0))

solve_btn.bind("<Enter>", _on_solve_enter)
solve_btn.bind("<Leave>", _on_solve_leave)
...
# Details panel below graph with border
...
# Create scrollbar for details text
details_scrollbar = ttk.Scrollbar(details_frame_container, orient="vertical", style="Vertical.TScrollbar")
details_scrollbar.pack(side="right", fill="y")

details_text = tk.Text(details_frame_container, height=12, wrap="word", font=("Segoe UI", 10),
